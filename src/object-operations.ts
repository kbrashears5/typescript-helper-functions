...
export class ObjectOperations {
     ...
     */
    public AreObjectsEqual(object1: object, object2: object, ...excludeList: string[]): boolean {
        const excludes = excludeList || [];

        const propNames = [... new Set(Object.getOwnPropertyNames(object1).concat(Object.getOwnPropertyNames(object2)))];

        const diffList = propNames.filter((column) => excludes.indexOf(column) < 0).filter((column) => !this.IsPropertyEqual(object1, object2, column));
     ...
     */
    public CopyProperty(source: any, target: any, propertyName: string, append: boolean = false) {
        if (this.IsNullOrEmpty(source)) throw new Error(`Value cannot be null or undefined: [source]`);
        if (this.IsNullOrEmpty(target)) throw new Error(`Value cannot be null or undefined: [target]`);
        if (this.IsNullOrWhitespace(propertyName)) throw new Error(`Value cannot be null or undefined: [propertyName]`);

        if (!source.hasOwnProperty(propertyName)) { return; }
     ...
     */
    public GetProperty<T>(source: any, propertyName: string, defaultValue?: T): T | undefined {
        if (this.IsNullOrEmpty(source)) throw new Error(`Value cannot be null or undefined: [source]`);
        if (this.IsNullOrWhitespace(propertyName)) { return defaultValue; }

        const key = Object.keys(source).find((k) => k.toLowerCase() === propertyName.toLowerCase());
    ...
    public IsMatch(pattern: string, value: string) {
        if (this.IsNullOrWhitespace(pattern)) throw new Error(`Value cannot be null or undefined: [pattern]`);
        ...
        if (pattern.startsWith('!') && pattern.substr(1) !== value) { return true; }
        if (pattern.endsWith('*') && value.startsWith(pattern.substr(0, pattern.length - 1))) { return true; }
        if (pattern.startsWith('*') && value.endsWith(pattern.substr(1))) { return true; }
     ...
     * @param propertyName {string} - Property name
     */
    public IsPropertyEqual(item1: any, item2: any, propertyName: string): boolean {
        if (this.IsNullOrEmpty(item1)) throw new Error(`Value cannot be null or undefined: [item1]`);
        if (this.IsNullOrEmpty(item2)) throw new Error(`Value cannot be null or undefined: [item2]`);
        if (this.IsNullOrWhitespace(propertyName)) throw new Error(`Value cannot be null or undefined: [propertyName]`);
    ...
    public MergeObjects<T extends AnyObject>(object1: T, object2: T) {
        ...
        if (object1) {
            Object.getOwnPropertyNames(object1)
                .forEach((name) => this.MergeObjectProperty(result, object1, object2, name));
        }
     ...
     * @param propertyName {string} - Property name.
     */
    public MergeObjectProperty<T extends AnyObject>(target: T, source1: T, source2: T, propertyName: string) {
        const value1 = (source1 && source1.hasOwnProperty(propertyName)) ? source1[propertyName] : undefined;

        const value2 = (source2 && source2.hasOwnProperty(propertyName)) ? source2[propertyName] : undefined;
     ...
     * @param append {boolean} - Indicates whether to add the property to the target object if it doesn't exist
     */
    public SetProperty(target: any, propertyName?: string, value?: any, append: boolean = true) {
        if (this.IsNullOrEmpty(target)) throw new Error(`Value cannot be null or undefined: [target]`);

        propertyName = propertyName || '';

        if (this.IsNullOrWhitespace(propertyName)) { return; }
